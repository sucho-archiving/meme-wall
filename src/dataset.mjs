...
import { memeMediaFolder, cacheDir } from "./config.mjs";
import {
  fetchMemes,
...
const loadCache = () => {
  ...
  log.info(` --> Loading cached dataset from ${cachePath}...`);
...
const saveCache = (data) => {
  ...
  log.info(` --> Saved dataset cache to ${cachePath}`);
};
...
if (SKIP_FETCH) {
  ...
  memes = cached.memes.map((m) => ({ ...m, timestamp: new Date(m.timestamp) }));
  metadataHierarchies = cached.metadataHierarchies;
  memeTypes = cached.memeTypes;
  people = cached.people;
  languages = cached.languages;
  countries = cached.countries;
  templateTypes = cached.templateTypes;
  groupOrders = cached.groupOrders;
} else {
  // fetch minimally-parsed data from the spreadsheet(s)
  let start = performance.now();
  log.info(" --> Fetching data from Google Sheets...");
  memes = await fetchMemes();
  metadataHierarchies = await fetchMetadataHierarchies();
  log.info(
  ...
  memes = memes
    .map((meme) => ({
      ...
      templateTypes: meme.memeTemplateType
        .split(", ")
        .map((type) => type.trim()),
  ...
  memeTypes = [
    ...
    .map((memeType) => ({
      ...
      group:
        Object.entries(metadataHierarchies.memeTypes).find(
          ([group, memeTypes]) => memeTypes.includes(memeType),
        )?.[0] || "Other",
    }))
    .sort((a, b) =>
      a.value
